...
function calendarDate(){
    ...
    $.ajax({
      type    : "GET",
      url     : "${pageContext.request.contextPath }/restful/englishVillage/calendarData",
      dataType: "json",
      data: ({
      ...
      success : function(data){
          $.each(data.itmes, function(i,item){
        	  if(item.COUNT < 5){
        		  $('#'+item.DAY+'day').html(item.DAY+"<span class='completeMarkEv01'>"+item.COUNT+"</span>");
        	  }else if(4 < item.COUNT && item.COUNT < 10){
        		  var random = Math.floor(Math.random()*10) + 1;
        		  if(random%2)
        		   mark = 'completeMarkEv02a';
        		  else mark = 'completeMarkEv02b';
        		  $('#'+item.DAY+'day').html(item.DAY+"<span class='"+mark+"'>"+item.COUNT+"</span>");
        	  }else if(9 < item.COUNT){
        		  $('#'+item.DAY+'day').html(item.DAY+"<span class='completeMarkEv03'>"+item.COUNT+"</span>");
        	  }
          });
      },
...
<form>
    ...
    <input type="hidden" name="pageName" value="/usr/englishVillage/calendar" />
    ...
    <div class="smartCont" role="main">
      <div class="recordTitle">
        <hgroup class="lessonRecH typeEv">
          ...
          <h3><spring:message code="englishvill.calender_text,p_1" text=""/></h3>
        </hgroup>
        <div class="btnView"><a href="javascript:doSubmit('${pageContext.request.contextPath }/usr/englishVillage/lessonsList','lessonsList')"><img src="<spring:message code="ima.view_list_btn" text=""/>" alt="list"></a></div>
      </div>
