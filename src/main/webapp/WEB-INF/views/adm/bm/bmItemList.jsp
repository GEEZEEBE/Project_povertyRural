	...
	<div id="tabs">
		<ul>
			<li><a
				...
				class=hover>Item</a></li>
			<li><a
				href='${pageContext.request.contextPath }/adm/billing/priceList'>Product</a></li>
		</ul>
		<div id="tabs-1">
				...
				<input type="hidden" name="searchAssetType" value="${requestMap.searchAssetType }" />
				<input type="hidden" name="searchItemName" value="${requestMap.searchItemName }" />
				</form>
			<!-- 검색조건 : srt -->
				...
				<table class="boardSch" summary="search">
					...
					<tr>
						...
						<td rowspan="2">
							<p class="btnSching">
								<button type="button"
									onClick="fncSerTxt(); doSubmit2('${pageContext.request.contextPath }/adm/billing/priceItemList', 'command2', 'post');">Search</button>
							</p>
						</td>
					</tr>
					<tr>
						<th>Asset Type</th>
						<td class="type">
							<select style="height: 24px;" name="searchAssetType" id="searchAssetType">
							<option value="">All</option>
							<c:forEach var="result" items="${godsDvsn}">
							<option value="${result.COMMON_CODE_ID }" <c:if test="${result.COMMON_CODE_ID==requestMap.searchAssetType}"> selected </c:if> >${result.CODE_NAME }</option>
							</c:forEach>
							</select>
						</td>
						<th>Item</th>
						<td>
							<p class="sch">
								<input type="text" name="searchItemName" title="검색어 입력" value="${requestMap.searchItemName}" />
							</p>
						</td>
					</tr>
				</table>
			        ...
			        <c:if test="${list == null || fn:length(list) == 0}">
						...
						<c:forEach var="result" items="${list}" varStatus="status"
							...
							<tr>
								...
								<td>${result.ASSET_TYPE_NM }</td>
								<td class="alignL">${result.CNTS_NAME }</td>
...
<div class="pageNum">
	<nav:pageNavi
		doSubmit="doSubmit('${pageContext.request.contextPath }/adm/billing/priceItemList', 'command', 'post');"
		pageList="${list}" />
</div>

<script>
	//fgetSelectTag('${pageContext.request.contextPath}/ws/common/commonCode','00600','searchAssetType','${requestMap.searchAssetType }','');
</script>